...
_Last updated: 2026-05-31. This doc is the single source of truth for resuming work. Read it top to bottom._
...
- **State:** A large **playful redesign + CMS/admin upgrade is complete and on `main`** (`e62f13c`). `main` and `redesign` are identical and pushed.
- **⚠️ Deploy lag:** The newest commits are merged to `main` on GitHub but **not yet deployed** — Vercel's free-tier **daily build limit** was hit. Production currently serves an earlier-but-complete deploy (`0affe4e`, the full redesign + homepage). The rest deploys via the scheduled job below (or a manual redeploy).
...
- **Bound so far: `/medicine` and `/about`** (full text + per-page SEO). Pattern to migrate another page: add a schema entry + make the page `async` + replace strings with `f("key", "exact fallback")`. Long-term cleaner home = a dedicated `page_content` table (needs a Supabase migration).

**SEO controls (wired):** `lib/seo.ts`. Root layout default title/description ← Settings→SEO (`getSeoDefaults`, safe fallbacks). Per-page SEO ← `pages` row via `buildPageMetadata` (applied on /medicine + /about; their pages rows were seeded to current values so nothing regressed). Other pages still use their own hardcoded `export const metadata` (works, just not admin-editable yet).

---

## Recommended next steps
1. **Bind the Home page** text (same pattern as Medicine/About).
2. **Per-page SEO sweep** on remaining pages with rows (outdoors, work, unplugged, path, cv, contact): seed each `pages` row to its current title/desc + swap `export const metadata` for `generateMetadata` calling `buildPageMetadata`. (Home has a bespoke long title — handle carefully.)
3. **balance / technology / philosophy** have NO `pages` row — create one (now easy via "New page") before binding/SEO.
...
- **Scripts** (`scripts/`, run with `node`): `seed-placements.mjs`, `seed-about-placements.mjs`, `seed-draft-posts.mjs` — patterns for seeding via service role.
